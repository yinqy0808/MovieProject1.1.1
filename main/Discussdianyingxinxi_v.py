...
def discussdianyingxinxi_sendemail(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
...
def discussdianyingxinxi_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select inteltype from storeup where userid = %d"%(request.session.get("params").get("id"))+" and tablename = 'discussdianyingxinxi' order by addtime desc")
        ...
        L = []
        cursor.execute("select * from discussdianyingxinxi where $intelRecomColumn in ('%s"%("','").join(leixing)+"') union all select * from discussdianyingxinxi where $intelRecomColumn not in('%s"%("','").join(leixing)+"')")
...
def discussdianyingxinxi_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        ...
        sql = ''
        if timeStatType == '日':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, sum({1}) total FROM discussdianyingxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d') LIMIT 10".format(xColumnName, yColumnName, where, '%Y-%m-%d')

        if timeStatType == '月':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, sum({1}) total FROM discussdianyingxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m') LIMIT 10".format(xColumnName, yColumnName, where, '%Y-%m')

        if timeStatType == '年':
            sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, sum({1}) total FROM discussdianyingxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y') LIMIT 10".format(xColumnName, yColumnName, where, '%Y')
...
def discussdianyingxinxi_count(request):
    '''
    总数接口
    '''
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT count(*) AS count FROM discussdianyingxinxi {0}".format(where)
        count = 0
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def discussdianyingxinxi_group(request, columnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT COUNT(*) AS total, " + columnName + " FROM discussdianyingxinxi " + where + " GROUP BY " + columnName + " LIMIT 10"
...
def discussdianyingxinxi_groupby(request):
    '''
    ...
    if request.method in ["POST", "GET"]:
        ...
        # print(discussdianyingxinxi.getallcolumn(discussdianyingxinxi,discussdianyingxinxi))
        if __authSeparate__=="是":
            #print("req_dict==============>",req_dict)
            #如果当前表有userid字段,且通过身份认证
            if "userid"  in discussdianyingxinxi.getallcolumn(discussdianyingxinxi,discussdianyingxinxi) and request.session.get("params")!=None:
                #参数增加userid
                req_dict["userid"]=request.session.get("params").get("id")
        #如果当前表没有userid字段,且参数里有userid键值对
        if  "userid" not  in discussdianyingxinxi.getallcolumn(discussdianyingxinxi,discussdianyingxinxi) and "userid" in list(req_dict.keys()):
            #删除参数userid
            del req_dict["userid"]
        try:
            del req_dict["page"]
        ...
        #本次查询所有,求总记录数
        # alldata = discussdianyingxinxi.objects.filter(**req_dict). \
        ...
        datas = discussdianyingxinxi.objects.filter(**req_dict).\
...
def discussdianyingxinxi_deleterecords(request):
    '''
    按键值对参数添加删除记录
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code}
        req_dict = request.session.get("req_dict")
        error=discussdianyingxinxi.deletebyparams(discussdianyingxinxi,discussdianyingxinxi,req_dict)
        if error!=None:
            msg['code'] = crud_error_code
            msg['msg'] = error
        return JsonResponse(msg)
...
def discussdianyingxinxi_flist(request):
    '''
    查看所有开放的帖列表(无需登录)
    '''
    if request.method in ["POST", "GET"]:
...
def discussdianyingxinxi_list_id(request,id_):
    '''
    查看主贴和所有回帖内容(无需登录)
    '''
    if request.method in ["POST", "GET"]:
        ...
        datas = discussdianyingxinxi.retrieve(discussdianyingxinxi, discussdianyingxinxi)
...
def discussdianyingxinxi_alipay(request):
    if request.method in ["POST", "GET"]:
        alipay = AliPay(
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://127.0.0.1:8080/MovieProject/discussdianyingxinxi/notify',
            #notify_url=''
        )
...
def discussdianyingxinxi_notify(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
        out_trade_no = req_dict['out_trade_no']
        cursor = connection.cursor()

        return redirect('http://127.0.0.1:8080/MovieProject/admin/dist/index.html#/discussdianyingxinxi')