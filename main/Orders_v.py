...
def orders_count(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT count(*) AS count FROM orders {0}".format(where)
        count = 0
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def orders_groupby(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        # print(orders.getallcolumn(orders,orders))
        if __authSeparate__=="是":
            #print("req_dict==============>",req_dict)
            #如果当前表有userid字段,且通过身份认证
            if "userid"  in orders.getallcolumn(orders,orders) and request.session.get("params")!=None:
                #参数增加userid
                req_dict["userid"]=request.session.get("params").get("id")
        #如果当前表没有userid字段,且参数里有userid键值对
        if  "userid" not  in orders.getallcolumn(orders,orders) and "userid" in list(req_dict.keys()):
            #删除参数userid
            del req_dict["userid"]
        try:
            del req_dict["page"]
        ...
        #本次查询所有,求总记录数
        # alldata = orders.objects.filter(**req_dict). \
        ...
        datas = orders.objects.filter(**req_dict).\
...
def orders_deleterecords(request):
    '''
    按键值对参数添加删除记录
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code}
        req_dict = request.session.get("req_dict")
        error=orders.deletebyparams(orders,orders,req_dict)
        if error!=None:
            msg['code'] = crud_error_code
            msg['msg'] = error
        return JsonResponse(msg)
...
def orders_list_id(request,id_):
    '''
    查看主贴和所有回帖内容(无需登录)
    '''
    if request.method in ["POST", "GET"]:
        ...
        datas = orders.retrieve(orders, orders)
...
def orders_alipay(request):
    if request.method in ["POST", "GET"]:
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://127.0.0.1:8080/MovieProject/orders/notify',
            #notify_url=''
        )
...
def orders_notify(request):
    if request.method in ["POST", "GET"]:
        ...
        return redirect('http://127.0.0.1:8080/MovieProject/admin/dist/index.html#/orders')