...
def dianyingfenlei_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        records=dianyingfenlei.getbyparams(dianyingfenlei, dianyingfenlei, recordsParam)
        if len(records)<1:
            msg['code'] = 400
            msg['msg'] = '用户不存在'
            return JsonResponse(msg)
...
def dianyingfenlei_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        # 当前表也是有管理员权限的表
        if  __isAdmin__ == "是" and 'dianyingfenlei' != 'forum':
            if req_dict.get("userid") and 'dianyingfenlei' != 'chat':
                del req_dict["userid"]
        ...
        msg['data']['pageSize']  =dianyingfenlei.page(dianyingfenlei, dianyingfenlei, req_dict, request, q)
...
主要信息列表（如商品列表，新闻列表）中使用，显示最近点击的或最新添加的5条记录就行
    ...
    if request.method in ["POST", "GET"]:
        ...
        elif "browseduration"  in dianyingfenlei.getallcolumn(dianyingfenlei,dianyingfenlei):
...
def dianyingfenlei_update(request):
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
        if req_dict.get("mima") and "mima" not in dianyingfenlei.getallcolumn(dianyingfenlei,dianyingfenlei) :
            del req_dict["mima"]
        if req_dict.get("password") and "password" not in dianyingfenlei.getallcolumn(dianyingfenlei,dianyingfenlei) :
...
def dianyingfenlei_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        ...
        sql = ''
        if timeStatType == '日':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, sum({1}) total FROM dianyingfenlei {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d') LIMIT 10".format(xColumnName, yColumnName, where, '%Y-%m-%d')

        if timeStatType == '月':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, sum({1}) total FROM dianyingfenlei {2} GROUP BY DATE_FORMAT({0}, '%Y-%m') LIMIT 10".format(xColumnName, yColumnName, where, '%Y-%m')

        if timeStatType == '年':
            sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, sum({1}) total FROM dianyingfenlei {2} GROUP BY DATE_FORMAT({0}, '%Y') LIMIT 10".format(xColumnName, yColumnName, where, '%Y')
...
def dianyingfenlei_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, sum({1}) AS total FROM dianyingfenlei {2} GROUP BY {0} LIMIT 10".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
# （按值统计）时间统计类型(多)
def dianyingfenlei_valueMul(request, xColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": []}
        ...
        for item in req_dict['yColumnNameMul'].split(','):
            sql = ''
            if timeStatType == '日':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, sum({1}) total FROM dianyingfenlei {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d') LIMIT 10".format(xColumnName, item, where, '%Y-%m-%d')

            if timeStatType == '月':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, sum({1}) total FROM dianyingfenlei {2} GROUP BY DATE_FORMAT({0}, '%Y-%m') LIMIT 10".format(xColumnName, item, where, '%Y-%m')

            if timeStatType == '年':
                sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, sum({1}) total FROM dianyingfenlei {2} GROUP BY DATE_FORMAT({0}, '%Y') LIMIT 10".format(xColumnName, item, where, '%Y')
...
# （按值统计(多)）
def dianyingfenlei_o_valueMul(request, xColumnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": []}
        ...
        for item in req_dict['yColumnNameMul'].split(','):
            sql = "SELECT {0}, sum({1}) AS total FROM dianyingfenlei {2} GROUP BY {0} LIMIT 10".format(xColumnName, item, where)
            L = []
            cursor = connection.cursor()
            cursor.execute(sql)
            desc = cursor.description
...
def dianyingfenlei_count(request):
    '''
    总数接口
    '''
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT count(*) AS count FROM dianyingfenlei {0}".format(where)
        count = 0
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def dianyingfenlei_group(request, columnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}

        where = ' where 1 = 1 '

        sql = "SELECT COUNT(*) AS total, " + columnName + " FROM dianyingfenlei " + where + " GROUP BY " + columnName + " LIMIT 10"
...
def dianyingfenlei_groupby(request):
    '''
    ...
    if request.method in ["POST", "GET"]:
        ...
        # print(dianyingfenlei.getallcolumn(dianyingfenlei,dianyingfenlei))
        if __authSeparate__=="是":
            #print("req_dict==============>",req_dict)
            #如果当前表有userid字段,且通过身份认证
            if "userid"  in dianyingfenlei.getallcolumn(dianyingfenlei,dianyingfenlei) and request.session.get("params")!=None:
                #参数增加userid
                req_dict["userid"]=request.session.get("params").get("id")
        #如果当前表没有userid字段,且参数里有userid键值对
        if  "userid" not  in dianyingfenlei.getallcolumn(dianyingfenlei,dianyingfenlei) and "userid" in list(req_dict.keys()):
            #删除参数userid
            del req_dict["userid"]
        try:
            del req_dict["page"]
        ...
        #本次查询所有,求总记录数
        # alldata = dianyingfenlei.objects.filter(**req_dict). \
        ...
        datas = dianyingfenlei.objects.filter(**req_dict).\
...
def dianyingfenlei_deleterecords(request):
    '''
    按键值对参数添加删除记录
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code}
        req_dict = request.session.get("req_dict")
        error=dianyingfenlei.deletebyparams(dianyingfenlei,dianyingfenlei,req_dict)
        if error!=None:
            msg['code'] = crud_error_code
            msg['msg'] = error
        return JsonResponse(msg)
...
def dianyingfenlei_flist(request):
    '''
    查看所有开放的帖列表(无需登录)
    '''
    if request.method in ["POST", "GET"]:
...
def dianyingfenlei_list_id(request,id_):
    '''
    查看主贴和所有回帖内容(无需登录)
    '''
    if request.method in ["POST", "GET"]:
        ...
        datas = dianyingfenlei.retrieve(dianyingfenlei, dianyingfenlei)
...
def dianyingfenlei_alipay(request):
    if request.method in ["POST", "GET"]:
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://127.0.0.1:8080/MovieProject/dianyingfenlei/notify',
            #notify_url=''
        )
...
def dianyingfenlei_notify(request):
    if request.method in ["POST", "GET"]:
        ...
        return redirect('http://127.0.0.1:8080/MovieProject/admin/dist/index.html#/dianyingfenlei')